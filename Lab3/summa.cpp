// Compile: mpiicpc -g -Wall -std=c++11 summa.c -o summa
// Run: srub --np <number of procs> ./summa <dim>
// <number of procs> must be perfect square, 4, 8, 16, 32, 64
// <dim> must be dividable by sqrt(<number of procs>)
...
// A[m,n], B[n,k], C[m,k]
// NOTE: for this assignment, we have simplified it such that m == n == k => dim, which MUST be divisible by the num procs.
int m;
int n;
int k;
...
// You should should not modify any code in this function.
// It is prepared for you to save your time.
    ...
    if (argc != 2) {
        if (myrank == 0) {
            fprintf(stderr, "USAGE:\n"
                    "srun --n <number of procs> ./summa <dim>\n"
